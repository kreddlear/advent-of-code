...
def determine_based_on_operator(num1, num2, operator):
    ...
    if operator == '+':
        ...
        answer = concatenate([num1,num2])
    # print("answer is", answer)
    return answer

def recurse_check_mult(solution, num_list, ops_list):
    # print("checking",solution, num_list, ops_list)
    if solution % num_list[-1] != 0:
        # print(f"return false, solution {solution} not divisible by last num {num_list[-1]}")
        return False
    elif ops_list[-2] == '*':
        # print(f"divisible by {num_list[-1]}")
        # if it is divisible and also the next one is a *
        # remove last num, remove last ops_list
        solution = solution // num_list[-1]
        num_list.pop()
        ops_list.pop()
        # check that too
        # print(f"recursing with", num_list, ops_list, solution)
        return recurse_check_mult(solution, num_list, ops_list)
    return True

def recurse_check_concat(solution, num_list, ops_list):
    num_to_check = str(num_list[-1])[-1]    
    sol_to_check = str(solution)[-1]
    if num_to_check != sol_to_check:
        # skip trying this combo, go to next iteration
        return False
    elif ops_list[-2] == '||':
        # if it is divisible and also the next one is a *
        # remove last num, remove last ops_list
        # remove last num from the end of the solution
        slice = 0 - len(str(num_list[-1]))
        # print("slice is ", slice)
        solution = str(solution)[:slice]
        num_list.pop()
        ops_list.pop()
        # check that too
        # print("fine so far, trying", num_list, ops_list, solution)
        return recurse_check_concat(int(solution), num_list, ops_list)
    return True

# content = [(7290, [6,8,6,15])]
# recurse_check_concat(123615, [6,8,6,15], ['*','||','||'])
...
for solution, nums in content:
    ...
    if solution == sum(nums):
        ...
        can_be_true_sum += solution
        continue
    elif solution == multiply(nums):
        # can_be_true.append(solution)
        can_be_true_sum += solution
        continue
    elif solution == concatenate(nums):
        # can_be_true.append(solution)
        can_be_true_sum += solution
        continue
    else:
        ...
        for combo in all_combos:
            # print("now checking combo", combo)
            # TODO add recursive checks if the last (then next to last, etc) op is *, check the solution is divisible by the last num
            if combo[-1] == '*':
                if recurse_check_mult(solution, list(nums), list(combo)) == False:
                    # print(f"giving up on {combo}, solution {solution} not divisible by last num {nums[-1]}")
                    # skip trying this combo, go to next iteration
                    continue
            if combo[-1] == '||':
                if recurse_check_concat(solution, list(nums), list(combo)) == False:
                    # print(f"giving up, solution {str(solution)[-1]} last num doesn't match last num {str(nums[-1])[-1]}")
                    # skip trying this combo, go to next iteration
                    continue
            # TODO same for concat
            ...
            for i in range(0, len(nums)-1):
                ...
                if i == 0:
                    ...
                    if i == len(nums)-2 and result == solution:
                        # print("we did it! with combo", combo)
                        # can_be_true.append(solution)
                        can_be_true_sum += solution
                        solved = True
                        break
